...
class MinimapElement {
    ...
    attachedCallback() {
        if (typeof atom.views.pollDocument === 'function') {
            this.subscriptions.add(atom.views.pollDocument(() => {
                this.pollDOM()
            }))
        } else {
    ...
    initializeContent() {
        ...
        this.subscriptions.add(this.subscribeTo(this.getFrontCanvas(), {
            'mousedown': (e) => {
                this.canvasPressed(this.extractMouseEventData(e))
            },
            'touchstart': (e) => {
                this.canvasPressed(this.extractTouchEventData(e))
            }
        }))
    }
    ...
    createVisibleArea() {
        ...
        this.visibleAreaSubscription = this.subscribeTo(this.visibleArea, {
            'mousedown': (e) => {
                this.startDrag(this.extractMouseEventData(e))
            },
            'touchstart': (e) => {
                this.startDrag(this.extractTouchEventData(e))
            }
        })
